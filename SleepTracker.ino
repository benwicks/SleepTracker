...
void loop()
{
  ...
  while (doRecordData) {
    ...
    if (!file.open(&root, "DATALOG.CSV", O_CREAT | O_APPEND | O_WRITE)) {
      Serial.println("Opening datalog.txt for append failed");
      digitalWrite(redLEDPin, HIGH);
    } else {
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (strstr(clientline, "GET / ") != 0) {
          ...
          client.stop();
          break;
        } else if (strstr(clientline, "GET /r.html") != 0) {
          client.println("HTTP/1.1 200 OK");
          client.println("Content-Type: text/plain");
          client.println();
          if (doRecordData) {
            client.println("<h2>Currently Recording</h2>");
          } else {
            client.println("<h2>NOT Recording</h2>");
          }
          delay(10);
          client.stop();
          break;
